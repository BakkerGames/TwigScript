...
public partial class Twig
{
        ...
        try
        {
            ...
            switch (token)
            {
                ...
                case ADDLIST:
                    ...
                    temp1 = Get($"{LIST_PREFIX}{p[0]}");
                    if (string.IsNullOrEmpty(temp1))
                        temp1 = p[1];
                    else
                        temp1 += "," + p[1];
                    Set($"{LIST_PREFIX}{p[0]}", temp1);
                    return;
                case ADDTO:
                    ...
                    return;
                case ANSWER:
                    // get an answer, to be handled by a script
                    CheckParamCount(token, p, 1);
                    _answerScript = p[0].StartsWith('@') ? p[0] : "@script(" + p[0] + ")";
                    Subchannel += SUBCHANNEL_ANSWER;
                    return;
                case CLEARARRAY:
                    // clears the named array
                    CheckParamCount(token, p, 1);
                    var keys = _dict.Keys.Where(x => x.StartsWith($"{ARRAY_PREFIX}{p[0]}"));
                ...
                case GETARRAY:
                    ...
                    temp1 = Get($"{ARRAY_PREFIX}{p[0]}.{int1}");
                    if (temp1 != "")
                    {
                ...
                case INSERTATLIST:
                    ...
                    Set($"{LIST_PREFIX}{p[0]}", string.Join(',', templist));
                    return;
                case ISNULL:
                ...
                case SETARRAY:
                    ...
                    Set($"{ARRAY_PREFIX}{p[0]}.{int1}", string.Join(',', templist));
                    return;
                case SETLIST:
                    // set a name,x,value
                    CheckParamCount(token, p, 3);
                ...
                case YORN:
                    CheckParamCount(token, p, 3);
                    // set the three internal script variables for HandleYorn()
                    _yornYesScript = p[0].StartsWith('@') ? p[0] : "@script(" + p[0] + ")";
                    _yornNoScript = p[1].StartsWith('@') ? p[1] : "@script(" + p[1] + ")";
                    _yornInvalidScript = p[2].StartsWith('@') ? p[2] : "@script(" + p[2] + ")";
    ...
    private void SkipToElse(string[] tokens, ref int index)
    {
        try
        {
            int level = 0;
    ...
    private void SkipPastEndif(string[] tokens, ref int index)
    {
        try
        {
            int level = 0;
    ...
    private void HandleForEach(List<string> p, string[] tokens, ref int index, StringBuilder result)
    {
